...
namespace CMU462 {
  ...
  {
    // Implement Me! (Task 4)
	  HalfedgeCIter vertex_he;
	  vertex_he = this->halfedge();

	  double alpha, beta;
	  Vector3D i_pos, j_pos, alpha_pos, beta_pos;
	  float u_i, u_j, lap_out, cot_alpha, cot_beta;
	  ...
	  do
	  {
		  ...
		  j_pos = vertex_he->next()->vertex()->position;
		  alpha_pos = vertex_he->twin()->next()->next()->vertex()->position;
		  beta_pos = vertex_he->next()->next()->vertex()->position;
		  
		  //Offset of the neighboring vertex
		  u_j = vertex_he->next()->vertex()->offset;
		  
		  //alpha and beta angles
		  alpha = acos(dot((i_pos - alpha_pos).unit(), (j_pos - alpha_pos).unit()));
		  beta = acos(dot((i_pos - beta_pos).unit(), (j_pos - beta_pos).unit()));

		  lap_out += 0.5f *(u_j - u_i) *(1.f / tanf(alpha) + 1.f / tanf(beta));
		  vertex_he = vertex_he->twin()->next();
	  } while (vertex_he != this->halfedge());
