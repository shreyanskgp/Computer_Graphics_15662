          ...
          if (mesh != nullptr) {
            if (action != Action::MeanCurvFlow){
              for (int i = 0; i < 2; i++) {
				...
				mesh->MeanCurvatureFlow(timestep,laplacian_type==LaplacianType::Cotan ? 0:1, volume_preservation==VolumePreservation::Yes ? 1:0);
              break;
            }
          }
        }
...
void Application::updateWidgets()
{
  ...
  if (mode == MODEL_MODE) {
    ...
    } else if (action == Action::MeanCurvFlow) { // Derek add
      if( scene->selected.object != scene->elementTransform &&
          scene->selected.element != nullptr )  {
        scene->elementTransform->setTarget( scene->selected );
      }
        ...
        switch(key)
        {
          ...
          case GLFW_KEY_SPACE:
            if (event == GLFW_PRESS && (action != Action::Raytrace_Video && action != Action::Rasterize_Video)) {
              ...
              setGhosted(!timeline.isCurrentlyPlaying() && action != Action::MeanCurvFlow);
            }
            break;
          case GLFW_KEY_LEFT:
            switch (mods) {
          ...
          case GLFW_KEY_W:
            to_wave_action();
            break;
          case GLFW_KEY_Q: // Derek add: key event for Mean curvature flow
            to_MCF_action();
            break;
          case GLFW_KEY_Z:
            for (auto o : scene->objects) {
          ...
          case GLFW_KEY_S:
            ...
            break;
		  case GLFW_KEY_Y: //Added by Shreyans
			  if (event == GLFW_PRESS) {
				  laplacian_type = LaplacianType::Cotan;
			  }
			  break;
		  case GLFW_KEY_H: //Added by Shreyans
			  if (event == GLFW_PRESS) {
				  laplacian_type = LaplacianType::One_Four;
			  }
		  case GLFW_KEY_0: //Added by Shreyans
			  if (event == GLFW_PRESS) {
				  volume_preservation = VolumePreservation::No;
			  }
			  break;
		  case GLFW_KEY_1: //Added by Shreyans
			  if (event == GLFW_PRESS) {
				  volume_preservation = VolumePreservation::Yes;
			  }
			  break;
...
void Application::to_MCF_action(){ // Derek add
  scene->elementTransform->exitObjectMode();
  scene->elementTransform->enterTransformedMode();
  scene->elementTransform->setTranslate();
  scene->removeObject(scene->elementTransform);
  action = Action::MeanCurvFlow;
  laplacian_type = LaplacianType::Cotan;
  volume_preservation = VolumePreservation::Yes;
  setGhosted(false);
}
    ...
    case LEFT:
      ...
      else if (mode == ANIMATE_MODE)
      {
        ...
        if (timeline.mouse_over(mouseX, mouseY)) {
          ...
          if (action == Action::MeanCurvFlow) { // Derek add
            if (scene->selected.element != nullptr && scene->selected.element->getInfo()[0][0] != 'V') {
              scene->selected.element = nullptr;
              scene->selected.object = nullptr;
            }
...
void Application::draw_action()
{
  ...
  switch( action )
  {
     ...
     case ( Action::Wave ):
        actionString << "Wave";
        break;
     case ( Action::MeanCurvFlow ): // Derek add
        actionString << "Mean Curvature Flow";
        break;
     case ( Action::Object ):
  ...
  if (mode == ANIMATE_MODE) {
	...
	//Added by Shreyans
	stringstream laplacian_string;
	laplacian_string << "Laplacian: ";
	switch (laplacian_type) {
	case LaplacianType::Cotan:
		laplacian_string << "Cotan-weighted";
		break;
	case LaplacianType::One_Four:
		laplacian_string << "Simple";
		break;
	}
	//Added by Shreyans
	stringstream vol_pres_string;
	vol_pres_string << "Preserve Volume: ";
	switch (volume_preservation) {
	case VolumePreservation::Yes:
		vol_pres_string << "Yes";
		break;
	case VolumePreservation::No:
		vol_pres_string << "No";
		break;
	}
	...
	draw_string(x0, y + 100, laplacian_string.str(), size, integrator_color);
	draw_string(x0, y + 120, vol_pres_string.str(), size, integrator_color);
  }
