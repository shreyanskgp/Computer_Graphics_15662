...
namespace CMU462 {
  ...
  Spectrum PathTracer::trace_ray(const Ray &r) {
	  ...
	  if (r.depth == 0 || isect.bsdf->is_delta() || r.origin_is_delta)
	  {
	  ...
	  for (SceneLight* light : scene->lights)
	  {
		  ...
		  double scale = 1.0 / (double)num_light_samples;
		  for (int i = 0; i < num_light_samples; i++) {
			  ...
			  double cos_theta = std::max(0., w_in[2]);
			  ...
			  //if (shadow_ray.max_t != EPS_D) cout << shadow_ray.max_t << endl;
			  //if (shadow_yes) cout << shadow_inter.t << endl;
			  if (!shadow_yes)
			  {
				  L_out = L_out + f*(float)cos_theta*(1.f / (float)pdf)*(float)scale*light_L;
			  }

			  //else if (shadow_inter.t<= EPS_D)
			  {
				  //L_out = L_out + f*cos_theta*(1. / pdf)*scale*light_L;
			  }
			  //else if (shadow_inter.primitive!=isect.primitive)
			  //{
			  //L_out = L_out + f*cos_theta*(1. / pdf)*scale*light_L;
			  //}*/
	  ...
	  ray_il.origin_is_delta = isect.bsdf->is_delta();
	  float terminateProbability = max(0.f, 1.f - il.illum());


	  //cout << terminateProbability << endl;
	  //Russian roulette and depth test
	  if (((float)rand()) / ((float)RAND_MAX) < terminateProbability || r.depth >= 4)
	  {
		  return L_out;
	  }
  ...
  Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
    ...
    // The sample rate is given by the number of camera rays per pixel.
	  ...
	  //If number of samples per pixel is >1
	  if (num_samples > 1)
	  {
		  for (int i = 0; i < num_samples; i++)
		  {
			  rand_2d = gridSampler->get_sample();
			  p.x = double((double)x + rand_2d.x) / double(frameBuffer.w);
			  p.y = double((double)y + rand_2d.y) / double(frameBuffer.h);
			  one_ray = one_ray + trace_ray(camera->generate_ray(p.x, p.y));
		  }
		  one_ray = one_ray*(1.f / float(num_samples));
		  return one_ray;
	  }
	  //printf("Ganda");
	  //Use center of pixel is number of samples is one
	  p.x = double((double)x + 0.5) / double(frameBuffer.w);
	  p.y = double((double)y + 0.5) / double(frameBuffer.h);
