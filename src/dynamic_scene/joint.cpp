...
namespace CMU462 { namespace DynamicScene {
   ...
   void Joint :: calculateAngleGradient( Joint* goalJoint, Vector3D q )
   {
     // Implement Me! (task 2B)
	   Vector3D temp_ang_grad, p;
	   vector<Vector3D> r;


	   //Vector from base point of the joint to the end point of the target joint
	   //p = goalJoint->getEndPosInWorld() - getBasePosInWorld();

	   //r is the axis of rotation
	   //r = cross(p, q - goalJoint->getEndPosInWorld());
	   //r = r.normalize;
	   
	   //getAxes(r);
	   Vector3D goalJoint_position = goalJoint->getEndPosInWorld();

	   for (Joint* j = this; j->parent != nullptr; j = j->parent)
	   {
		   p = goalJoint_position - j->getBasePosInWorld();
		   j->getAxes(r);
		   temp_ang_grad[0] = dot(cross(r[0].unit(), p), goalJoint_position - q);
		   temp_ang_grad[1] = dot(cross(r[1].unit(), p), goalJoint_position - q);
		   temp_ang_grad[2] = dot(cross(r[2].unit(), p), goalJoint_position - q);
		   j->ikAngleGradient += temp_ang_grad;
	   }

	   //temp_ang_grad[0] = dot(cross(r[0].unit(), p), goalJoint->getEndPosInWorld() - q);
	   //temp_ang_grad[1] = dot(cross(r[1].unit(), p), goalJoint->getEndPosInWorld() - q);
	   //temp_ang_grad[2] = dot(cross(r[2].unit(), p), goalJoint->getEndPosInWorld() - q);
	   //ikAngleGradient = temp_ang_grad;
	   
	   //cross(r[1], p)[1], cross(r[2], p)[2]);
	   
	   
	   
	  // cout << goalJoint << "Ganda" << endl;
   }
   ...
   Vector3D Joint::getBasePosInWorld()
   {
     ...
     */
     ...
     return (T * Vector4D(0.,0.,0.,1.)).projectTo3D();
   }
   ...
   Vector3D Joint::getEndPosInWorld()
   {
     ...
     */
	   ...
	   //Applying joint transformation and then translation
	   return (T * SceneObject::getTransformation() * Matrix4x4::translation(axis)) * Vector4D(0., 0., 0., 1.).projectTo3D();
