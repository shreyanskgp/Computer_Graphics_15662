...
double Mesh::computePointLineSegmentDistance(Vector3D P, Vector3D P0, Vector3D P1){
  // Point P, Line Segment P0P1
  Vector3D v = P1 - P0;
  Vector3D w = P  - P0;
  Vector3D distVec;
  double c1 = dot(w, v);
  ...
  if (c1 <= 0) { // P is before P0
    distVec = P - P0;
    return distVec.norm();
  }
  else if (c2 <= c1) { // P is after P1
    distVec = P - P1;
    return distVec.norm();
  }

  double b = c1/c2;
  Vector3D P_project = P0 + b * v;
  distVec = P - P_project;
  return distVec.norm();
}
...
void Mesh::linearBlendSkinning(bool useCapsuleRadius)
{
  ...
  // Initialization
  int numVert = mesh.nVertices();
  int numJoints = skeleton->joints.size();
  int jointCount;
  Vector3D vertPosTemp, P0, P1;
  vector<LBSInfo> LBSs(numJoints);
  double sumInvDist;
  Matrix4x4 T;
  ...
  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
    ...
    // Compute LBS for vertex_i
    sumInvDist = 0;
    for (jointCount=0; jointCount<numJoints; jointCount++){
      Joint* j = skeleton->joints[jointCount];
      T = j->getBindTransformation();
      P0 = (0, 0, 0);
      P1 = j->axis;
      LBSs[jointCount].distance = computePointLineSegmentDistance(T.inv()*v->bindPosition, P0, P1);
      LBSs[jointCount].blendPos = T.inv() * v->bindPosition;
      if (useCapsuleRadius == false){
        sumInvDist += 1/LBSs[jointCount].distance;
      }
      if (LBSs[jointCount].distance <= j->capsuleRadius && useCapsuleRadius == true){
        sumInvDist += 1/LBSs[jointCount].distance;
      }
    }
    if (sumInvDist == 0) sumInvDist = 1; // avoid devided by 0

    vertPosTemp = (0,0,0);
    for (jointCount=0; jointCount<numJoints; jointCount++){
      Joint* j = skeleton->joints[jointCount];
      T = j->getTransformation();
      if (useCapsuleRadius == false){
        vertPosTemp += (1/LBSs[jointCount].distance) / sumInvDist * (T*LBSs[jointCount].blendPos);
      }
      if (LBSs[jointCount].distance <= j->capsuleRadius && useCapsuleRadius == true){
        vertPosTemp += (1/LBSs[jointCount].distance) / sumInvDist * (T*LBSs[jointCount].blendPos);
      }
    }
    if (vertPosTemp == (0,0,0)) vertPosTemp = v->position; // handle the case of no joint in the radius
    v->position = vertPosTemp;
  }
}
...
void Mesh::forward_euler(float timestep, float damping_factor) {
  // Implement Me! (Task 4)
  float ddu;
  vector<float> laplaceList(mesh.nVertices());
  int vertCount = 0;
  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
    laplaceList[vertCount] = v->laplacian();
    vertCount++;
  }
  vertCount = 0;
  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
    ddu = laplaceList[vertCount] - damping_factor*v->velocity;
    v->offset = v->offset + timestep*v->velocity;
    v->velocity = v->velocity + timestep*ddu;
    vertCount++;
  }
}
...
double Mesh::computeMeshVolume(){
  ...
  for (FaceIter f = mesh.facesBegin(); f != mesh.facesEnd(); f++) {
    v1 = f->halfedge()->vertex()->position;
    v2 = f->halfedge()->next()->vertex()->position;
    v3 = f->halfedge()->next()->next()->vertex()->position;
    v321 = v3.x*v2.y*v1.z;
    v231 = v2.x*v3.y*v1.z;
    v312 = v3.x*v1.y*v2.z;
    v132 = v1.x*v3.y*v2.z;
...
void Mesh::MeanCurvatureFlow(float timestep, int lap_type, int vol_pres){
  // Project Implementation 
  int vertCount, numVert = mesh.nVertices();
  double dualArea = 0.0;
  Vector3D vec1, vec2;
  HalfedgeIter h;
  double volumeOld, volumeNew;
  volumeOld = computeMeshVolume();

  // ======================================================
  // Step1: Construct map (key:VertexIter, value:vertexIdx)
  map<VertexIter, int> vertexMap;
  vertCount = 0;
  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
    vertexMap[v] = vertCount;
    vertCount++;
  }
  // Step2: Construct inv(massMatrix) 
  Eigen::SparseMatrix<double> invMassMat(numVert, numVert);
    for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
    h = v->halfedge();
    // Compute Dual Area
    dualArea = 0.0;
    do{
      vec1 = h->twin()->vertex()->position - h->vertex()->position;
      vec2 = h->next()->twin()->vertex()->position - h->next()->vertex()->position;
      dualArea += cross(vec1, vec2).norm() / 6;
      h = h->twin()->next();
    }
    while(h != v->halfedge());
    // invMassMat(?,?) = 1/dualArea
    invMassMat.insert(vertexMap.find(v)->second, vertexMap.find(v)->second) = 1.0 / dualArea;
  }
  invMassMat = invMassMat / invMassMat.sum() * numVert;

  // ======================================================
  // Step3: Constrct Laplace matrix
  Eigen::SparseMatrix<double> cotanLaplaceMat(numVert, numVert);
  Vector3D vi, vj, va, vb;
  float cotAlpha, cotBeta, cotSum;

  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
    h = v->halfedge();
    cotSum = 0.0;
    do{
      vi = h->vertex()->position;
      vj = h->twin()->vertex()->position;
      va = h->next()->next()->vertex()->position;
      vb = h->twin()->next()->next()->vertex()->position;
      cotAlpha = dot(vj-va, vi-va) / cross(vj-va, vi-va).norm();
      cotBeta  = dot(vi-vb, vj-vb) / cross(vi-vb, vj-vb).norm();
	  cotanLaplaceMat.coeffRef(vertexMap.find(v)->second, vertexMap.find(v)->second) += (lap_type == 0 ? (cotAlpha + cotBeta) / 2. : 1.);
      cotanLaplaceMat.coeffRef(vertexMap.find(v)->second, vertexMap.find(h->twin()->vertex())->second) -= (lap_type ==0 ? (cotAlpha + cotBeta) /2. : 1.);
      h = h->twin()->next();
    }
    while(h != v->halfedge());
  }
  ...
  // ======================================================
  // Step4: Construct (I - inv(M)*h*L)
  Eigen::SparseMatrix<double> MCFMatrix(numVert, numVert);
  // MCFMatrix = - timestep / 10 * cotanLaplaceMat; 
  MCFMatrix = - invMassMat * (timestep )  * cotanLaplaceMat;
  // Add identity matrix
  for (int i = 0; i < numVert; i++) MCFMatrix.coeffRef(i,i) += 1;

  // ======================================================
  // Step5: Solve Linear system
  Eigen::VectorXd vertOld_x(numVert), vertOld_y(numVert), vertOld_z(numVert); 
  Eigen::VectorXd vertNew_x(numVert), vertNew_y(numVert), vertNew_z(numVert);
  // Construce old vertex positions vector for solving linear system
  vertCount = 0;
  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
    vertOld_x[vertCount] = v->position.x;
    vertOld_y[vertCount] = v->position.y;
    vertOld_z[vertCount] = v->position.z;
    vertCount++;
  }
  // Sparse solver
  // Eigen::SimplicialLLT<Eigen::SparseMatrix<double>> solver;
  // solver.compute(MCFMatrix);
  // vertNew_x = solver.solve(vertOld_x);
  // vertNew_y = solver.solve(vertOld_y);
  // vertNew_z = solver.solve(vertOld_z);

  // forward euler
  vertNew_x = MCFMatrix * vertOld_x;
  vertNew_y = MCFMatrix * vertOld_y;
  vertNew_z = MCFMatrix * vertOld_z;

  // ======================================================
  // Step6: Update mesh position
  vertCount = 0;
  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
    v->position.x = vertNew_x[vertCount];
    v->position.y = vertNew_y[vertCount];
    v->position.z = vertNew_z[vertCount];
    vertCount++;
  }
  // preserve volume
  volumeNew = computeMeshVolume();
  vertCount = 0;
  if (vol_pres != 0)
  {
	  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
		  v->position.x = v->position.x * pow(volumeOld / volumeNew, 1.0 / 3.0);
		  v->position.y = v->position.y * pow(volumeOld / volumeNew, 1.0 / 3.0);
		  v->position.z = v->position.z * pow(volumeOld / volumeNew, 1.0 / 3.0);
		  vertCount++;
	  }
  }
