  ...
  void Skeleton::unkeyframe(double t) {
   ...
   void Skeleton :: reachForTarget(map<Joint*, Vector3D> targets, double time )
   {
     ...
     // Do several iterations of Jacobian Transpose gradient descent for IK
	   ...
	   //typedef typename map<Joint*, Vector3D>::iterator target_iter;
	   
	   double time_step = 0.01;
	   int num_of_iter = 300;

	   //cout << targets.begin()->first->getBasePosInWorld() << targets.begin()->first->getEndPosInWorld() << endl;
	   for (int iter_counter = 0; iter_counter < num_of_iter; iter_counter++)
	   {
		   //Setting all ikAngleGradients to zero
		   for (auto j = joints.begin(); j!= joints.end(); j++)
		   {
			   j[0]->ikAngleGradient = 0.;
		   }

		   for (auto i = targets.begin(); i != targets.end(); i++) //Accumulating all the gradients
		   {
			   i->first->calculateAngleGradient(i->first, i->second);
		   }

		   for (auto j = joints.begin(); j != joints.end(); j++)
		   {
			   j[0]->setAngle(time,j[0]->getAngle(time) - time_step*j[0]->ikAngleGradient);
			   j[0]->rotation = j[0]->getAngle(time);
		   }

			   /*for (Joint* j = i->first; j->parent != nullptr; j = j->parent)
			   {
				   j->calculateAngleGradient(i->first, i->second);
